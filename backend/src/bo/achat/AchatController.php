    ...
    function doListInventaireAchat($request) {
    	try {
    		...
    		if (isset($request['iDisplayStart']) && isset($request['iDisplayLength'])) {
    			...
    			// End filter from dataTable
    			$achats = $achatManager->retrieveAchatInventaire($request['dateDebut'], $request['dateFin'], $request['regle'],
    					  $request['codeUsine'],$request['iDisplayStart'], $request['iDisplayLength'], $sOrder, $sWhere);
    			if ($achats != null) {
    				$nbAchats = $achatManager->count($request['dateDebut'], $request['dateFin'], $request['regle'],
    					  $request['codeUsine'],$request['iDisplayStart'], $request['iDisplayLength'], $sOrder, $sWhere);
    				$this->doSuccessO($this->dataTableFormat($achats, $request['sEcho'], $nbAchats));
    			} else {
    				$this->doSuccessO($this->dataTableFormat(array(), $request['sEcho'], 0));
