   ...
   public function retrieveAll($typeAchat,$codeUsine,$offset, $rowCount, $orderBy = "", $sWhere = "") {
    ...
    public function getInfoPoidsTotal($typeAchat, $dateDebut, $dateFin, $codeUsine) {
        ...
        if($typeAchat=='*')
            ...
            $sql = 'SELECT SUM(poidsTotal) poidsTotal FROM achat a, reglement_achat ra WHERE achat_id=a.id AND status=1 and regle='.$typeAchat.' and codeUsine="'.$codeUsine.'" and date(dateAchat) between "'.$dateDebut.'" and "'.$dateFin.'"';
        $stmt = Bootstrap::$entityManager->getConnection()->prepare($sql);
    	$stmt->execute();
    	$infos = $stmt->fetch();
    	return $infos;
